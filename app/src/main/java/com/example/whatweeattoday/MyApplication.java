...
import static com.example.whatweeattoday.Constants.MAX_BYTES_PDF;
...
public class MyApplication extends Application {
    ...
    public static void deleteRecipe(Context context, String recipeId, String recipeUrl, String recipeTitle ) {
        String TAG = "DELETE_RECIPE_TAG";

        Log.d(TAG, "deleteRecipe: deleting...");

        ProgressDialog progressDialog = new ProgressDialog(context);
        progressDialog.setTitle("Please wait");
        progressDialog.setMessage("Deleting"+recipeTitle+" ...");; //e.g. Deleting Recipe ABC...
        progressDialog.show();

        Log.d(TAG, "deleteRecipe: Deleting from storage...");
        StorageReference storageReference = FirebaseStorage.getInstance().getReferenceFromUrl(recipeUrl);
                        ...
                        pdfView.fromBytes(bytes)
                                ...
                                .onLoad(new OnLoadCompleteListener() {
                                    ...
                                    public void loadComplete(int nbPages) {
                                        ...
                                        if(pagesTv != null){
                                            pagesTv.setText(""+nbPages); //concatenate with double quotes, can't set int in TextView
                                        }
                                    }
                                })
                                .load();
    ...
    public static void incrementRecipeViewCount(String recipeId) {
        //1. get recipe views count
    ...
    public static void downloadRecipe(Context context, String recipeId, String recipeTitle, String recipeUrl) {
        Log.d(TAG_DOWNLOAD, "downloadRecipe: downloading recipe...");

        String nameWithExtension = recipeTitle + ".pdf";
        Log.d(TAG_DOWNLOAD, "downloadRecipe: NAME: "+nameWithExtension);

        //progress dialog
        ProgressDialog progressDialog = new ProgressDialog(context);
        progressDialog.setTitle("Please wait");
        progressDialog.setMessage("Downloading "+ nameWithExtension +"..."); //e.g. downloading ABC_Recipe.pdf
        ...
        storageReference.getBytes(MAX_BYTES_PDF)
                .addOnSuccessListener(new OnSuccessListener<byte[]>() {
                    @Override
                    public void onSuccess(byte[] bytes) {
                        Log.d(TAG_DOWNLOAD, "onSuccess: Recipe Downloaded");
                        Log.d(TAG_DOWNLOAD, "onSuccess: Saving recipe...");
                        saveDownloadedRecipe(context, progressDialog, bytes, nameWithExtension, recipeId);
                    }
                })
                .addOnFailureListener(new OnFailureListener() {
                    @Override
    ...
    private static void saveDownloadedRecipe(Context context, ProgressDialog progressDialog, byte[] bytes, String nameWithExtension, String recipeId) {
        Log.d(TAG_DOWNLOAD, "saveDownloadedRecipe: Saving downloaded recipe");
        try {
            ...
            Log.d(TAG_DOWNLOAD, "saveDownloadedRecipe: Saved to Download Folder");
            progressDialog.dismiss();

            incrementRecipeDownloadCount(recipeId);
        }
        catch (Exception e) {
            Log.d(TAG_DOWNLOAD, "saveDownloadedRecipe: Failed saving to Download Folder due to "+e.getMessage());
            Toast.makeText(context,"Failed saving to Download Folder due to "+e.getMessage(), Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
        }
    }

    private static void incrementRecipeDownloadCount(String recipeId) {
        Log.d(TAG_DOWNLOAD, "incrementRecipeDownloadCount: Incrementing Recipe Download Count");
        ...
        ref.child(recipeId)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Recipe");
                        reference.child(recipeId).updateChildren(hashMap)
                                .addOnSuccessListener(new OnSuccessListener<Void>() {
                                    @Override
                                    public void onSuccess(Void unused) {
    ...
    public static void addToFavorite(Context context, String recipeId){
        ...
        if(firebaseAuth.getCurrentUser() == null) {
            ...
            //setup data to add in firebase db of current user for favorite recipe
            HashMap<String, Object> hashMap = new HashMap<>();
    ...
    public static void removeFromFavorite(Context context, String recipeId){
        //remove if user is logged in
        //1.check if user is logged in
        FirebaseAuth firebaseAuth = FirebaseAuth.getInstance();
        if(firebaseAuth.getCurrentUser() == null) {
            //not logged in, can't remove to fav
            Toast.makeText(context,"You're not logged in", Toast.LENGTH_SHORT).show();
        }
        else {
            //remove from db
